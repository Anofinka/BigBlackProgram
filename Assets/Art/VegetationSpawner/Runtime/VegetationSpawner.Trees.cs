            ...
            if (item.enabled)
            {
                ...
                foreach (Vector3 pos in item.spawnPoints)
                {
                    ...
                    if (autoRespawnTrees == false && showProgressBar)
                    {
                        ...
                        if (spawnPointCount > 1 && counter % (spawnPointCount / 10) == 0)
                        {
                            UnityEditor.EditorUtility.DisplayProgressBar("Vegetation Spawner", $"Spawning \"{item.name}\" on {terrain.name} ({counter}/{spawnPointCount} spawn points)", (float)counter / (float)spawnPointCount);
                        }
                    }
#endif
                    ...
                    float aspect = Vector3.Dot(-dir, normal);
