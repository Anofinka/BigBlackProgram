...
namespace sc.terrain.vegetationspawner
{
    ...
    public class VegetationSpawnerInspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                EditorGUILayout.LabelField("Version " + VegetationSpawner.Version, EditorStyles.largeLabel);

                if (GUILayout.Button(new GUIContent(" Utilities window", EditorGUIUtility.IconContent("TerrainInspector.TerrainToolSettings").image)))
                {
                    VegetationUtilitiesWindow.ShowWindow();
                }
            }
            ...
            if (spawner.terrains.Count > 0)
            {
                ...
                if (TabID != 3 && visibleTerrainsOnly.boolValue)
                {
                    EditorGUILayout.HelpBox("Only spawning on terrains visible in the scene view", MessageType.Info);
                }
            ...
            using (var grassChange = new EditorGUI.ChangeCheckScope())
            {
                ...
                if (grass != null)
                {
                    ...
                    EditorGUI.BeginDisabledGroup(!grass.enabled);
                    {
                        ...
                        grass.type = (SpawnerBase.GrassType)EditorGUILayout.Popup("Render type", (int)grass.type, new[]
                        {
                            "Mesh", "Texture"
                        }, GUILayout.Width(EditorGUIUtility.labelWidth + 80f));
